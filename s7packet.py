...
class S7Packet:
    ...
    def parse(self):
        ctr = 0
        self.ROSCTR_val = self.s7_packet[1]
        ...
        if self.s7_header[4] > 2:  # s7_header[4] is param_length and [5] is data_length
            if self.ROSCTR_val == 7:  # Userdata packets have extended parameter sections and different PDUs
                self.param_type = int(self.s7_param_bytes[5]) >> 4  # Get the first 4 bits of this number
                if self.param_type == 4:  # Request type
                    size_left = int(self.s7_param_bytes[3]) - 4
                    fmt_string = '!3sBBBBB'
                    if size_left > 0:
            ...
            if self.function_code != 29 and self.function_code != 30 and self.function_code != 31:  # upload functions don't have data items
                for i in range(0, self.item_count):
                    ...
                    self.item_contents.append(
                        unpack(item_data_fmt_string, self.s7_PDU_bytes[current:current + item_length]))
    ...
    def print_details(self):
        ...
        print()
        print("s7 header          : {}".format(binascii.hexlify(self.s7_header_bytes)))  # print hex of s7_header_bytes
        ...
        if self.s7_header[4] > 0:  # only do this if we have some parameters!
            print("s7 parameter area  : {}".format(binascii.hexlify(self.s7_param_bytes)))
        if self.ROSCTR_val == 1 or self.ROSCTR_val == 2 or self.ROSCTR_val == 3:
            if hasattr(self, 'items'):
                for index, item in enumerate(self.items):
                    print("  item {} address   : {}".format(index + 1, item[7]))
        elif self.ROSCTR_val == 7:
            print("  Parameter head   : {}".format(binascii.hexlify(self.param_head)))
            print("  Sequence number  : {}".format(self.sqn_number))
        if self.s7_header[5] > 0:
            print("s7 PDU             : {}".format(binascii.hexlify(self.s7_PDU_bytes)))
            if hasattr(self, 'item_contents'):
                for index, item in enumerate(self.item_contents):
                    print("  item {} data   : {}".format(index + 1, item[0]))
        else:
            print("s7 PDU             : None")
